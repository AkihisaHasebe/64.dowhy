"""GES (Greedy Equivalence Search) 因果探索モジュール"""
...
def run_ges(data, column_names, target, score_func="local_score_BIC",
            maxP=None, n_bootstrap=100, threshold=0.50):
    """GES アルゴリズム + Bootstrap を実行する。
    ...
    for _ in range(n_bootstrap):
        ...
        boot_data = data[idx]
        rec_b = ges(boot_data, score_func=score_func, maxP=maxP,
                    node_names=column_names)
        adj_b = rec_b["G"].graph
        for i in range(n):
            for j in range(n):
